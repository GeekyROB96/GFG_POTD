...
class Solution
{
    ...
    int printKDistantfromLeaf(Node root, int k)
    {
        ...
        helper(root,al,s,k,0);
        
        return s.size() ;
    }
    
    public void helper(Node root, ArrayList<Node> al, Set<Node> s, int k, int level){
        ...
        al.add(root);
        helper(root.left,al,s,k,level+1);
        helper(root.right,al,s,k,level+1);
        al.remove(al.size()-1);
    }
